...
public final class JavaFileObjectSubjectTest {
    ...
    private static final JavaFileObject CLASS = JavaFileObjects.forSourceLines(// 
    "test.TestClass", "package test;", "", "public class TestClass {}");

    private static final JavaFileObject DIFFERENT_NAME = JavaFileObjects.forSourceLines(// 
    "test.TestClass2", "package test;", "", "public class TestClass2 {}");

    private static final JavaFileObject CLASS_WITH_FIELD = JavaFileObjects.forSourceLines(// 
    "test.TestClass", "package test;", "", "public class TestClass {", "  Object field;", "}");

    private static final JavaFileObject UNKNOWN_TYPES = JavaFileObjects.forSourceLines("test.TestClass", "package test;", "", "public class TestClass {", "  Bar badMethod(Baz baz) { return baz.what(); }", "}");

    private static final JavaFileObject SAMPLE_ACTUAL_FILE_FOR_MATCHING = JavaFileObjects.forSourceLines("test.SomeFile", "package test;", "", "import pkg.AnAnnotation;", "import static another.something.Special.CONSTANT;", "", "@AnAnnotation(with = @Some(values = {1,2,3}), and = \"a string\")", "public class SomeFile {", "  private static final int CONSTANT_TIMES_2 = CONSTANT * 2;", "  private static final int CONSTANT_TIMES_3 = CONSTANT * 3;", "  private static final int CONSTANT_TIMES_4 = CONSTANT * 4;", "", "  @Nullable private MaybeNull field;", "", "  @Inject SomeFile() {", "    this.field = MaybeNull.constructorBody();", "  }", "", "  protected int method(Parameter p, OtherParam o) {", "    return CONSTANT_TIMES_4 / p.hashCode() + o.hashCode();", "  }", "", "  public static class InnerClass {", "    private static final int CONSTANT_TIMES_8 = CONSTANT_TIMES_4 * 2;", "", "    @Nullable private MaybeNull innerClassField;", "", "    @Inject", "    InnerClass() {", "      this.innerClassField = MaybeNull.constructorBody();", "    }", "", "    protected int innerClassMethod(Parameter p, OtherParam o) {", "      return CONSTANT_TIMES_8 / p.hashCode() + o.hashCode();", "    }", "  }", "}");
    ...
    public void hasContents_failure() {
        expectFailure.whenTesting().about(javaFileObjects()).that(CLASS_WITH_FIELD).hasContents(JavaFileObjects.asByteSource(DIFFERENT_NAME));
