...
public class TreeContextTest {
    ...
    private static final ImmutableList<String> baseTreeSource = ImmutableList.of("package test;", "", "final class TestClass {", "    public String toString() {", "        Object variable = new Object();", "        return \"" + LITERAL_VALUE + "\" + variable;", "    }", "", "    public void nonsense() {", "        int[] numbers = {0, 1, 2, 3, 4};", "        for (int x : numbers) {", "            if (x % 2 == 0) {", "                throw new IllegalStateException();", "            }", "        }", "    }", "}");

    private static final ParseResult PARSE_RESULTS = MoreTrees.parseLines(baseTreeSource);

    private static final CompilationUnitTree COMPILATION_UNIT = PARSE_RESULTS.compilationUnits().iterator().next();
    ...
    private Tree compilationSubtree() {
        return MoreTrees.findSubtree(COMPILATION_UNIT, Tree.Kind.STRING_LITERAL, LITERAL_VALUE);
    }
    ...
    private int subtreeStartIdx() {
        ...
        throw new IllegalStateException(String.format("Couldn't find a literal with value \"%s\" in " + "the test tree source", LITERAL_VALUE));
    }

    private long subtreeStartLine() {
        // Line numbers start at 1
        return subtreeStartIdx() + 1;
    }
    ...
    private Tree invalidNode() {
        return MoreTrees.parseLinesToTree("package test;", "", "final class OtherClass {}");
    }
    ...
    public void getNodePath() {
        assertThat(treeContext().getNodePath(compilationSubtree()).getCompilationUnit()).isEqualTo(COMPILATION_UNIT);
    }
}
