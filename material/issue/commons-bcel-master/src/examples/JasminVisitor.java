...
public class JasminVisitor extends org.apache.bcel.classfile.EmptyVisitor {
    ...
    public void visitJavaClass(final JavaClass clazz) {
        ...
        out.println("." + Utility.classOrInterface(clazz.getAccessFlags()) + " " + Utility.accessToString(clazz.getAccessFlags(), true) + " " + clazz.getClassName().replace('.', '/'));
    ...
    public void visitMethod(final Method method) {
        // Remember for use in subsequent visitXXX calls
        this._method = method;
     ...
     */
        ...
        for (final InstructionHandle ih1 : ihs) {
            if (ih1 instanceof BranchHandle) {
                ...
                if (bi instanceof Select) {
                    // Special cases LOOKUPSWITCH and TABLESWITCH
        ...
        for (final LocalVariableGen l : lvs) {
            out.println(".var " + l.getIndex() + " is " + l.getName() + " " + l.getType().getSignature() + " from " + get(l.getStart()) + " to " + get(l.getEnd()));
        }
        out.print("\n");
        ...
        for (final CodeExceptionGen c : ehs) {
            ...
            final String class_name = // catch any exception, used when compiling finally
            (caught == null) ? "all" : caught.getClassName().replace('.', '/');
