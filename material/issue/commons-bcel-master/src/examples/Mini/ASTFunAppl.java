...
/**
 */
public class ASTFunAppl extends ASTExpr implements MiniParserTreeConstants, org.apache.bcel.Constants {

    private ASTIdent name;

    // Points to Function in environment
    ...
    public int eval(final int expected) {
        ...
        int t = fun.getType();
        // Only true if all arguments are simple expressions
        is_simple = true;
        // Check arguments
        if (exprs != null) {
            for (int i = 0; i < exprs.length; i++) {
                // length match checked in previous pass
                // May be T_UNKNOWN
                final int expect = args[i].getType();
                // May be T_UNKNOWN
                final int t_e = exprs[i].eval(expect);
                if ((expect != T_UNKNOWN) && (t_e != expect)) {
                    MiniC.addError(exprs[i].getLine(), exprs[i].getColumn(), "Argument " + (i + 1) + " in application of " + fname + " is not of type " + TYPE_NAMES[expect] + " but " + TYPE_NAMES[t_e]);
                } else {
                    // Update, may be identical
                    args[i].setType(t_e);
                }
                // Check condition
        ...
        if (t == T_UNKNOWN) {
            // May be still T_UNKNOWN
            fun.setType(t = expected);
        }
        return type = t;
    }
