...
public abstract class Utility {
    ...
    public static String accessToString(final int access_flags, final boolean for_class) {
        ...
        int p = 0;
        for (int i = 0; p < Const.MAX_ACC_FLAG_I; i++) {
    ...
    public static String codeToString(final ByteSequence bytes, final ConstantPool constant_pool, final boolean verbose) throws IOException {
        ...
        switch(opcode) {
            ...
            case Const.INVOKEINTERFACE:
                ...
                buf.append("\t").append(constant_pool.constantToString(index, Const.CONSTANT_InterfaceMethodref)).append(verbose ? " (" + index + ")\t" : "").append(nargs).append("\t").append(// Last byte is a reserved space
                bytes.readUnsignedByte());
                break;
            case Const.INVOKEDYNAMIC:
                index = bytes.readUnsignedShort();
                buf.append("\t").append(constant_pool.constantToString(index, Const.CONSTANT_InvokeDynamic)).append(verbose ? " (" + index + ")\t" : "").append(// Thrid byte is a reserved space
                bytes.readUnsignedByte()).append(// Last byte is a reserved space
                bytes.readUnsignedByte());
                break;
                ...
                case 'L':
                    {
                        ...
                        // Look for closing `;'
                        final int index = signature.indexOf(';', fromIndex);
                        if (index < 0) {
                            throw new ClassFormatException("Invalid signature: " + signature);
                        }
