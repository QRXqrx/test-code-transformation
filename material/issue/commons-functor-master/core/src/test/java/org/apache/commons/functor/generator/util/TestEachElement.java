...
public class TestEachElement extends BaseFunctorTest {
    ...
    @Test
    public void testFromNull_1() {
        /*[Transform from] assertNull(EachElement.from((Collection<?>) null));*/
        EachElement.from((Collection<?>) null);
        assertNull(EachElement.from((Map<?, ?>) null));
    }

    @Test
    public void testFromNull_2() {
        /*[Transform from] assertNull(EachElement.from((Collection<?>) null));*/
        EachElement.from((Collection<?>) null);
        /*[Transform from] assertNull(EachElement.from((Map<?, ?>) null));*/
        EachElement.from((Map<?, ?>) null);
        assertNull(EachElement.from((Iterator<?>) null));
    }

    @Test
    public void testFromNull_3() {
        /*[Transform from] assertNull(EachElement.from((Collection<?>) null));*/
        EachElement.from((Collection<?>) null);
        /*[Transform from] assertNull(EachElement.from((Map<?, ?>) null));*/
        EachElement.from((Map<?, ?>) null);
        /*[Transform from] assertNull(EachElement.from((Iterator<?>) null));*/
        EachElement.from((Iterator<?>) null);
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void testWithMap_1() {
        List<?> col = (List<?>) EachElement.from(map).toCollection();
        int i = 0;
        for (; i < col.size(); i++) {
            Map.Entry<String, String> entry = (Map.Entry<String, String>) col.get(i);
            if (entry.getKey().equals("1")) {
                /*[Transform from] assertEquals("1-1", entry.getValue());*/
                entry.getValue();
            } else if (entry.getKey().equals("2")) {
                /*[Transform from] assertEquals("2-1", entry.getValue());*/
                entry.getValue();
            } else if (entry.getKey().equals("3")) {
                /*[Transform from] assertEquals("3-1", entry.getValue());*/
                entry.getValue();
            } else if (entry.getKey().equals("4")) {
                /*[Transform from] assertEquals("4-1", entry.getValue());*/
                entry.getValue();
            } else if (entry.getKey().equals("5")) {
                /*[Transform from] assertEquals("5-1", entry.getValue());*/
                entry.getValue();
            }
        }
        assertEquals(5, i);
    ...
    @Test
    public void testWithStop_1() {
        /*[Transform from] assertEquals("[0, 1, 2]", new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString());*/
        new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString();
        assertEquals("[0, 1, 2, 3]", new GenerateUntil<Integer>(EachElement.from(list), new Offset(3)).toCollection().toString());
    }

    @Test
    public void testWithStop_2() {
        /*[Transform from] assertEquals("[0, 1, 2]", new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString());*/
        new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString();
        /*[Transform from] assertEquals("[0, 1, 2, 3]", new GenerateUntil<Integer>(EachElement.from(list), new Offset(3)).toCollection().toString());*/
        new GenerateUntil<Integer>(EachElement.from(list), new Offset(3)).toCollection().toString();
        assertEquals("[0, 1, 2]", new WhileGenerate<Integer>(new Limit(3), EachElement.from(list)).toCollection().toString());
    }

    @Test
    public void testWithStop_3() {
        /*[Transform from] assertEquals("[0, 1, 2]", new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString());*/
        new UntilGenerate<Integer>(new Offset(3), EachElement.from(list)).toCollection().toString();
        /*[Transform from] assertEquals("[0, 1, 2, 3]", new GenerateUntil<Integer>(EachElement.from(list), new Offset(3)).toCollection().toString());*/
        new GenerateUntil<Integer>(EachElement.from(list), new Offset(3)).toCollection().toString();
        /*[Transform from] assertEquals("[0, 1, 2]", new WhileGenerate<Integer>(new Limit(3), EachElement.from(list)).toCollection().toString());*/
        new WhileGenerate<Integer>(new Limit(3), EachElement.from(list)).toCollection().toString();
